...
class Plotter:
    ...
    def initialize(self):
        ...
        if "line" in self.type:
            self.fig_line, self.ax_line = plt.subplots(xlim=xlim,ylim=ylim)
        ...
        if "circle" in self.type:
            ...
            self.particles, = self.ax_circle.plot([], [], '{}o'.format(self.fill), ms=6)
    ...
    def connect(self):
        self.cid = self.fig_line.canvas.mpl_connect('button_press_event', self.onclick)
        print(self.cid)
